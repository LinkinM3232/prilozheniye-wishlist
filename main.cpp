...
1. Посмотреть все игры в списке - за это отвечает функция ViewAllGames, которая считывает данные из бинарного файла и выводит их в консоль.

2. Добавить игру - за это отвечает функция AddGame, а каждой каждой игре автоматом присваивается уникальный id

3. Удалить игру - за это отвечает функция DeleteGame, удаляются игры по id
...
<<<<<<< Updated upstream
=======
7. Первоначальная регистрация пользователя - RegisterInteractiveUser, а также ввод логина и пароля пользователем - LoginInteractiveLoop
